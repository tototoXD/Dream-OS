...
export class OpenAiDreamProvider extends DreamOSAiProvider {
  constructor({
    ...
    this.safetyIdentifier = crypto.createHash('sha256').update(String(safetyIdentifier)).digest('hex').slice(0, 32);
  }
  ...
  async request(instruction, payload) {
    ...
    const response = await fetch(`${this.baseUrl}/responses`, {
      ...
      body: JSON.stringify({
        ...
        text: { verbosity: 'low' },
        input: [
          { role: 'developer', content: `${safetyAndScope}\n\n${instruction}` },
    ...
    if (!response.ok) {
      const message = data?.error?.message || `OPENAI_API_${response.status}`;
      throw new Error(message);
    }
    return jsonText(responseText(data));
  }
  ...
  async transcribe({ audioBase64, mimeType = 'audio/webm' } = {}) {
    ...
    form.append('file', new Blob([bytes], { type: mimeType }), `dream.${mimeType.split('/')[1] || 'webm'}`);
    ...
    const data = await response.json().catch(() => ({}));
    if (!response.ok) throw new Error(data?.error?.message || `OPENAI_TRANSCRIPTION_${response.status}`);
    return { text: String(data?.text || ''), modelVersion: process.env.OPENAI_TRANSCRIPTION_MODEL || 'gpt-4o-mini-transcribe' };
  }
}
