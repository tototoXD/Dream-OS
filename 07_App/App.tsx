...
function Icon({ name, color = COLORS.ink }: { name: 'home' | 'dreams' | 'understanding' | 'graph' | 'mic' | 'arrow' | 'back' | 'calendar' | 'menu' | 'close'; color?: string }) {
  ...
  return <Svg width={size} height={size} viewBox="0 0 24 24" accessibilityLabel={name}>
...
export default function App() {
  ...
  return (
    <LinearGradient colors={['#e9e7ec', '#f7f5f4', '#efd0d0'] as const} start={{ x: 0, y: 0 }} end={{ x: 1, y: 1 }} style={styles.root}>
      ...
      <View style={[styles.orb, styles.orbOne]} /><View style={[styles.orb, styles.orbTwo]} /><View style={[styles.orb, styles.orbThree]} />
      {renderScreen()}
...
function HomeScreen({ aiLabel, onRecord, onOpenDrawer, height }: { aiLabel: string; onRecord: () => void; onOpenDrawer: () => void; height: number }) {
  ...
  return <View style={styles.screen}>
    <GlassBar style={styles.homeTop}><Pressable onPress={onOpenDrawer} accessibilityLabel="打开理解历史"><Icon name="menu" /></Pressable><Text style={styles.mode}>{aiLabel}</Text></GlassBar>
...
function UnderstandingScreen({ dreams, latest, onStart, onRecord }: { dreams: Dream[]; latest?: Dream; onStart: (dream: Dream) => void; onRecord: () => void }) {
  ...
  return <View style={styles.screen}><View style={styles.pageTop}><Text style={styles.pageTitle}>理解</Text><Text style={styles.subtitle}>近期状态</Text></View><ScrollView contentContainerStyle={styles.listContent}><View style={styles.stateCard}><Text style={styles.cardLabel}>近期状态 · {isDone ? '最近更新' : '基于当前记录'}</Text><Text style={styles.stateTitle}>{isDone ? '知道方向，但不容易推进' : '先看反复出现的线索'}</Text><Text style={styles.stateCopy}>{isDone ? '来自近期梦境 · 你已确认 · 阶段性参考' : `来自 ${dreams.length} 条梦境记录 · 阶段性参考`}</Text><View style={styles.stateDivider} /><Text style={styles.stateLabel}>反复出现</Text><Text style={styles.stateValue}>着急、等待、寻找出口</Text><Text style={styles.stateFoot}>{dreams.length} 条记录</Text></View><View style={[styles.stateCard, isDone && styles.confirmedState]}><Text style={styles.cardLabel}>{isDone ? '你确认过' : '尚未确认'}</Text><Text style={styles.stateTitle}>{isDone ? '知道目的地，却推进不了' : '完成澄清并选择“很贴近”后显示'}</Text><Text style={styles.stateFoot}>{isDone ? '你的反馈 · 本次澄清' : '尚未有用户确认'}</Text></View><View style={styles.observedState}><Text style={styles.cardLabel}>继续观察</Text><Text style={styles.stateValue}>工作推进</Text><Text style={styles.stateFoot}>尚未确认</Text></View><Text style={styles.sectionLabel}>最新梦境</Text><DreamNote dream={latest} />{!isDone && <View style={styles.latestAction}><Text style={styles.latestStatus}>{inProgress ? '理解未完成' : '尚未开始理解'}</Text><Pressable onPress={() => onStart(latest)} style={styles.primaryButton}><Text style={styles.primaryButtonText}>{inProgress ? (latest.sessionState === 'READY' ? '查看理解草稿' : '继续理解') : '开始理解'}</Text></Pressable></View>}{confirmed && confirmed.id !== latest.id && <Text style={styles.mutedNote}>最近一次确认：{confirmed.title}</Text>}</ScrollView></View>;
}
...
const COLORS = { ink: '#292824', muted: '#77737a', paper: '#fffdf8', canvas: '#ece9ed', line: 'rgba(82, 78, 89, .18)', glass: 'rgba(255,255,255,.68)' };

const styles = StyleSheet.create({
  root: { flex: 1, backgroundColor: COLORS.canvas, overflow: 'hidden' },
  screen: { flex: 1, paddingHorizontal: 22, paddingTop: Platform.OS === 'ios' ? 56 : 34, paddingBottom: 108 },
  ...
  sessionScreen: { paddingBottom: 94 }, sessionTop: { flexDirection: 'row', alignItems: 'center', gap: 12, marginBottom: 17 }, progress: { flex: 1, height: 4, backgroundColor: 'rgba(100,105,125,.18)', borderRadius: 4, overflow: 'hidden' }, progressFill: { height: 4, backgroundColor: COLORS.ink, borderRadius: 4 }, chatContent: { paddingBottom: 90 }, chatMessage: { maxWidth: '92%', borderRadius: 17, paddingHorizontal: 15, paddingVertical: 13, marginBottom: 13 }, aiMessage: { alignSelf: 'flex-start', backgroundColor: 'rgba(255,255,255,.48)', borderWidth: 1, borderColor: 'rgba(125,130,147,.2)' }, userMessage: { alignSelf: 'flex-end', backgroundColor: COLORS.ink, borderBottomRightRadius: 5 }, chatText: { color: COLORS.ink, fontSize: 15, lineHeight: 24 }, userText: { color: '#fff' }, typing: { color: COLORS.muted, fontSize: 13, paddingVertical: 8 }, optionList: { gap: 9, marginTop: 4 }, optionRow: { minHeight: 64, paddingHorizontal: 15, paddingVertical: 13, backgroundColor: 'rgba(255,255,255,.5)', borderWidth: 1, borderColor: 'rgba(122,128,148,.3)', borderRadius: 15, flexDirection: 'row', alignItems: 'center', justifyContent: 'space-between' }, optionPressed: { backgroundColor: 'rgba(255,255,255,.85)', transform: [{ scale: 0.99 }] }, optionCopy: { flex: 1, paddingRight: 12 }, optionLabel: { color: COLORS.ink, fontSize: 14, lineHeight: 20, fontWeight: '600' }, optionDetail: { color: COLORS.muted, fontSize: 12, lineHeight: 18, marginTop: 4 }, radio: { width: 19, height: 19, borderRadius: 10, borderWidth: 1.5, borderColor: '#888' }, resultAction: { minHeight: 44, backgroundColor: COLORS.ink, borderRadius: 12, alignItems: 'center', justifyContent: 'center', marginTop: 3, paddingHorizontal: 18 }, resultActionText: { color: '#fff', fontWeight: '600' }, sessionComposer: { position: 'absolute', left: 22, right: 22, bottom: 28, height: 51, flexDirection: 'row', alignItems: 'center', borderWidth: 1, borderColor: 'rgba(108,114,132,.32)', backgroundColor: 'rgba(255,255,255,.75)', borderRadius: 20, overflow: 'hidden', shadowColor: '#525970', shadowOpacity: 0.14, shadowRadius: 12, shadowOffset: { width: 0, height: 7 }, elevation: 4 }, voiceComposer: { borderColor: '#bd3a32' }, composerButton: { width: 46, height: 50, alignItems: 'center', justifyContent: 'center' }, composerInput: { flex: 1, height: 50, paddingHorizontal: 10, color: COLORS.ink }, sessionVoiceStatus: { position: 'absolute', bottom: 84, left: 40, right: 40, textAlign: 'center', color: '#bd3a32', fontSize: 11 },
  resultScreen: { paddingBottom: 30 }, simpleTop: { height: 40 }, resultContent: { paddingBottom: 100 }, resultKicker: { color: '#777', fontSize: 14, marginBottom: 9 }, resultCore: { color: COLORS.ink, fontSize: 30, lineHeight: 42, fontWeight: '600' }, resultTail: { color: '#555', fontSize: 15, marginTop: 8, marginBottom: 23 }, uncertain: { paddingVertical: 14, borderTopWidth: 1, borderBottomWidth: 1, borderColor: '#ddd9d0' }, uncertainLabel: { color: '#888', fontSize: 12, marginBottom: 4 }, uncertainText: { color: '#555', fontSize: 14, lineHeight: 23 }, evidenceToggle: { paddingVertical: 10 }, evidenceBox: { marginTop: 3, marginBottom: 16, padding: 14, backgroundColor: 'rgba(242,240,234,.82)', borderRadius: 12 }, evidenceItem: { paddingVertical: 5 }, evidenceLabel: { color: '#888', fontSize: 10, marginBottom: 4 }, evidenceQuote: { color: '#45423b', fontSize: 13, lineHeight: 21 }, feedbackTitle: { textAlign: 'center', fontSize: 18, color: COLORS.ink, marginTop: 24, marginBottom: 11 }, feedbackLinks: { flexDirection: 'row', justifyContent: 'center', gap: 22, marginTop: 10 }, disclaimer: { textAlign: 'center', color: '#888', fontSize: 11, lineHeight: 17, marginTop: 19 }, supplementMessage: { maxWidth: '90%', marginBottom: 12, padding: 10 }, supplementUser: { alignSelf: 'flex-end', backgroundColor: COLORS.ink, borderRadius: 16, paddingHorizontal: 13 },
  ...
  tabBar: { position: 'absolute', left: 12, right: 12, bottom: Platform.OS === 'ios' ? 20 : 12, height: 70, borderRadius: 28, flexDirection: 'row', alignItems: 'center', justifyContent: 'space-around', paddingHorizontal: 8 }, tab: { alignItems: 'center', justifyContent: 'center', minWidth: 62, gap: 2 }, tabLabel: { color: COLORS.muted, fontSize: 11 }, tabLabelActive: { color: COLORS.ink, fontWeight: '600' }, empty: { alignItems: 'center', paddingTop: 120 }, emptyTitle: { color: COLORS.ink, fontSize: 22, marginBottom: 9 }, emptyCopy: { color: COLORS.muted, fontSize: 14, marginBottom: 20 }, smallButton: { backgroundColor: COLORS.ink, borderRadius: 20, paddingHorizontal: 22, paddingVertical: 11 }, smallButtonText: { color: '#fff', fontSize: 13, fontWeight: '600' }, toast: { position: 'absolute', bottom: 95, alignSelf: 'center', paddingHorizontal: 15, paddingVertical: 10, borderRadius: 18, backgroundColor: COLORS.ink }, toastText: { color: '#fff', fontSize: 13 },
});

const liquidTabStyles = StyleSheet.create({
  active: { minHeight: 50, paddingHorizontal: 11, borderRadius: 22, backgroundColor: 'rgba(41,40,36,.9)', shadowColor: '#292824', shadowOpacity: 0.2, shadowRadius: 12, shadowOffset: { width: 0, height: 5 }, elevation: 4 },
});
...
const DOT_STYLE_OVERRIDES = {
  ...
  homeTop: { minHeight: 44, paddingHorizontal: 9, paddingVertical: 4, borderRadius: 23, marginBottom: 2 },
  pageTop: { minHeight: 46, paddingHorizontal: 8, paddingVertical: 4, borderRadius: 23, marginBottom: 20, backgroundColor: 'rgba(255,255,255,.52)', borderWidth: 1, borderColor: 'rgba(255,255,255,.82)', shadowColor: '#423d46', shadowOpacity: 0.1, shadowRadius: 18, shadowOffset: { width: 0, height: 8 }, elevation: 2 },
  dreamsHeader: { minHeight: 52, paddingHorizontal: 9, paddingVertical: 5, borderRadius: 26, marginBottom: 8, backgroundColor: 'rgba(255,255,255,.52)', borderWidth: 1, borderColor: 'rgba(255,255,255,.82)', shadowColor: '#423d46', shadowOpacity: 0.1, shadowRadius: 18, shadowOffset: { width: 0, height: 8 }, elevation: 2 },
  calendarHeader: { minHeight: 46, paddingHorizontal: 8, paddingVertical: 4, borderRadius: 23, marginBottom: 23, backgroundColor: 'rgba(255,255,255,.52)', borderWidth: 1, borderColor: 'rgba(255,255,255,.82)', shadowColor: '#423d46', shadowOpacity: 0.1, shadowRadius: 18, shadowOffset: { width: 0, height: 8 }, elevation: 2 },
  sessionTop: { minHeight: 46, paddingHorizontal: 8, paddingVertical: 4, borderRadius: 23, marginBottom: 17, backgroundColor: 'rgba(255,255,255,.52)', borderWidth: 1, borderColor: 'rgba(255,255,255,.82)', shadowColor: '#423d46', shadowOpacity: 0.1, shadowRadius: 18, shadowOffset: { width: 0, height: 8 }, elevation: 2 },
  simpleTop: { minHeight: 46, paddingHorizontal: 8, paddingVertical: 4, borderRadius: 23, marginBottom: 12, backgroundColor: 'rgba(255,255,255,.44)', borderWidth: 1, borderColor: 'rgba(255,255,255,.74)' },
  ...
  recordButton: { backgroundColor: 'rgba(41,40,36,.9)', borderColor: 'rgba(255,255,255,.28)', borderWidth: 1, shadowColor: '#292824', shadowOpacity: 0.24, shadowRadius: 18, shadowOffset: { width: 0, height: 9 }, elevation: 7 },
  ...
  userMessage: { backgroundColor: 'rgba(41,40,36,.92)', borderRadius: 17, borderBottomRightRadius: 5, paddingHorizontal: 14, paddingVertical: 11, alignSelf: 'flex-end', maxWidth: '88%', shadowColor: '#24221f', shadowOpacity: 0.12, shadowRadius: 8, shadowOffset: { width: 0, height: 4 }, elevation: 2 },
  ...
  evidenceBox: { backgroundColor: 'rgba(255,255,255,.28)', borderRadius: 0, borderLeftWidth: 2, borderLeftColor: 'rgba(116,108,115,.42)', paddingLeft: 14 },
  ...
  tabBar: { left: 14, right: 14, bottom: Platform.OS === 'ios' ? 20 : 14, height: 66, borderRadius: 33, paddingHorizontal: 7, backgroundColor: 'rgba(255,255,255,.56)', borderColor: 'rgba(255,255,255,.84)', shadowColor: '#423d46', shadowOpacity: 0.16, shadowRadius: 22, shadowOffset: { width: 0, height: 10 }, elevation: 7 },
};
...
const captureStyles = StyleSheet.create({
  ...
  captureSave: { alignSelf: 'center', minWidth: 156, minHeight: 46, marginTop: 16, paddingHorizontal: 22, borderRadius: 23, backgroundColor: 'rgba(41,40,36,.9)', borderWidth: 1, borderColor: 'rgba(255,255,255,.28)', alignItems: 'center', justifyContent: 'center', shadowColor: '#292824', shadowOpacity: 0.2, shadowRadius: 16, shadowOffset: { width: 0, height: 8 }, elevation: 5 },
