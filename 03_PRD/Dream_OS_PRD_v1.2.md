...
所有页面中的 Dream Record 都使用同一种便签卡片：首页记录、Dreams 历史、日历结果和 Understanding 中的梦境入口保持相同的便签背景、边框、轻微倾斜和时间标签。每个 Dream Record 只对应一张独立卡片，不使用叠层或错位纸张效果；标题为第一层级，日期与原样内容随后展示。AI 状态和心理理解必须使用独立的信息卡，不与梦境便签混用。
