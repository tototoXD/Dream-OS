...
点击“记录梦境”后不立即录音，而是进入便签页等待输入，并自动聚焦默认文字框、吊起键盘。便签正文顶部显示等待输入的光标；用户可以直接打字，或点击右下角麦克风开始语音输入；再次点击麦克风结束语音并将转写写入同一条 Dream Record。开始输入后首页引导文案隐藏，页面中部显示梦境记录便签。只有产生有效文字或完成一段有效语音后，页面才在便签卡片外部显示小尺寸“保存”按钮；保存按钮不得成为便签卡片内容的一部分。

梦境便签显示创建日期与时间，并提供明确的“保存”按钮。点击保存后，系统依据记录内容生成简短的描述性标题，例如“无法抵达的 B2”。自动标题用于识别记录，不得包含心理分析、象征解释或诊断；用户之后仍可修改标题。

保存前，页面左上角必须提供“返回”按钮，不将其放入梦境便签内部。返回只退出当前便签视图并保留本地草稿，不归档到 `Dreams`，也不删除内容；用户再次开始输入时恢复同一条草稿。删除草稿必须是单独且明确的操作。

保存完成后，Dream Record 归档到 `Dreams`。`Dreams` 保存用户历史上所有已保存梦境的原样记录，包括标题、完整记录时间、原始文字/转写和原始语音引用。AI 观察、心理假设或后续长期理解不得覆盖原样记录；保存后不再提供补充或编辑入口。

保存操作成功后，梦境便签通过缩小并朝底部 `Dreams` 导航收起的动画表达“已归档”，随后进入“梦已经保存”确认页，不自动开始理解。确认页直接展示刚保存的同一张梦境便签，并询问“现在开始理解吗？”。主操作为“现在理解”，次要操作为无边框文字链接“稍后理解”；用户选择前不得创建 Understanding Session。不额外显示 `Dreams` 红点，避免在已明确的保存反馈后增加干扰。系统遵循操作系统的“减少动态效果”设置，以短淡出替代明显的位移动画。

进入 `Understanding` 后，系统默认展示用户近一段时间的阶段性心理状态整理，而不是空白的分析入口。整理必须明确区分：反复记录到的体验、用户已确认的理解、仍在观察的关系及各自证据；不得将其呈现为诊断或固定结论。状态整理下方展示最新梦境，并邀请用户“开始澄清这个梦”。只有用户明确选择后才进入解析流程；选择“稍后再说”不得生成新的心理理解，页面保留该梦境的待理解状态。未完成会话离开后自动保存，并收纳到首页理解历史，用户从历史入口继续。
一条梦完成一次 `Understanding Session` 后，系统先生成 `DRAFT` 理解草稿，不得立即标记为已理解。只有用户明确选择“很贴近”并提交反馈后，Dream Record 才标记为 `UNDERSTOOD_ONCE`，进入理解历史，Dreams 卡片显示“已理解”且不再显示理解按钮。“部分贴近”“不太像”“还不能判断”均保留为未确认状态，不得写入稳定个人理解。Understanding 确认完成后，近期状态整理同步更新更新时间、相关确认内容和证据数量；最新梦境区域改为“最近理解”，不再显示“开始澄清这个梦”。

用户选择澄清最新梦境后，MVP 不要求用户预先选择“先理解哪一部分”。系统可以引用刚才已向用户展示的近期状态整理，但首问仍必须优先从本次梦境中可验证的直接体验与情绪开始，不以未经确认的象征解释作为首问。问题需要用具体描述帮助用户回到梦境现场；选项采用纵向单列表单，每个选项包含清晰的补充描述和右侧选择器，点击整行即发送。其余问题依据已获得的信息按“确认体验 → 关键时刻 → 现实背景”的顺序逐题询问；界面一次只显示一个问题，并允许用户跳过。系统在获得必要信息后才生成阶段性理解。

Understanding Session 采用 AI 聊天对话流。AI 直接发送观察、当前问题和可直接回答的候选选项，不显示“AI 问题”标签，不用卡片包裹 AI 文本；顶部只显示轻量进度条。点击候选选项即作为一条用户消息发送。用户也可以不使用候选项，通过底部统一输入栏自由打字或使用语音回答。候选项、文字与语音输入必须进入同一会话上下文和同一追问逻辑，不得被实现为互不连通的分支流程。AI 每次只推进一个问题；用户离开时自动保存，未完成内容收纳到首页理解历史，达到理解门槛后在对话内提供进入 Current Understanding 的入口。

`Dreams` 默认以可滚动卡片列表按记录时间倒序展示历史梦境。梦境一经保存即作为当时的原始记录，不再允许补充或跨日编辑。每张卡片只显示一种理解状态：尚未开始时显示“尚未理解”与“理解这个梦”；中途离开显示“理解未完成”与“继续理解”；完成并确认后只显示“已理解”，点击卡片查看原始梦境和理解结果。页面右上角提供日历入口，点击进入独立的“梦境日历”页面；有梦境记录的日期在数字下方显示圆点。点击有记录的日期后，页面在月历下方展示当天全部梦境卡片；无记录日期不可进入详情。圆点只表示当天存在至少一条记录，不代表分析完成度，也不展示心理判断。

所有页面中的 Dream Record 都使用同一种便签卡片：首页记录、保存确认页、Dreams 历史、日历结果和 Understanding 中的梦境入口保持相同的便签背景、边框、轻微倾斜和时间标签。待理解状态只改变上下文标签和操作（“现在理解”“开始理解”或“继续理解”），不改变梦境标题、时间和原样记录。每个 Dream Record 只对应一张独立卡片，不使用叠层或错位纸张效果；标题为第一层级，日期与原样内容随后展示。AI 状态和心理理解必须使用独立的信息卡，不与梦境便签混用。
...
MVP 阶段，进入 `RECORD_FIRST` 后不允许用户强制要求 AI 给出心理分析，也不重新开放原始梦境编辑。未来出现相似的新梦或新的独立记录时，系统再重新评估。这样可以避免产品在证据不足时退化成迎合式解梦工具。
...
AI 先生成一条用户能直接读懂的核心理解，再显示一条不确定信息；不输出长篇“解梦文章”。结果页固定为“一句话理解 + 一句不确定 + 一个依据入口 + 三个反馈操作”。依据默认收起，点击“为什么这样理解”后原地展开梦境与用户回答各一条。所有推断使用概率语言，并显示信息来源。
...
首屏最多显示三个行动项。点击“补充”后不新开流程，而是在当前结果下展开 AI 对话输入区；原理解保持可见。AI 只提示用户补充细节或自己的理解，不再提供三组选项。用户可自由打字或语音补充，AI 最多追问 1-2 次；对话中提供内联文字“重新整理这次理解”，不显示完成按钮。点击“很贴近”后显示短提示“已保存这次理解”，随后返回理解页并更新近期状态。
...
| `RECORD_FIRST` | 当前证据不足，先记录 | 缺少情绪、背景、个人联想或区分性证据 | 查看、完成、等待未来重评 | 新的独立记录触发 READINESS_CHECK |
...
stateDiagram-v2
    ...
    RECORD_FIRST --> READINESS_CHECK: 未来出现相似新梦
...
四个标签分别承载记录入口、原始梦境归档、AI 理解和长期自我模型。MVP 将 `Settings` 收入“自我”二级页面，避免移动端底部导航过度拥挤。详细页面、组件和状态规范见 `05_UX/Dream_OS_Page_Level_Wireframe_Spec_v1.0.md`。
...
- 首页左上角提供仅含菜单/历史图标的入口，打开“理解历史”抽屉。抽屉承载账号摘要、最近完成的理解和按梦境关联的历史 Understanding Session；账号摘要中的梦境数量可点击并复用 `Dreams` 查看全部原始记录，从该入口进入时显示返回箭头并返回原侧边栏，从底部 Tab 进入时不显示。理解历史不复制原始梦境正文。
...
- 首屏先展示“我的近期状态”：用一条阶段性总结、一个用户已确认体验、一个正在观察的现实联系和不超过三条变化摘要，给用户一个可读的心理建模入口；状态必须标注为阶段性参考，不得表现为诊断或固定人格结论；
- 提供“查看变化”详情页，以近 3 个月时间线呈现新出现、持续出现和比上个阶段更常出现的体验；每条变化必须能回溯到相关梦境，记录不足时不生成趋势结论；
- 以高频个人意象为中心展示轻量关系网络；
...
| D-10 | RECORD_FIRST 绕过 | MVP 不允许强制绕过；未来出现相似的新梦后再重新评估 |
