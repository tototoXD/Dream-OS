...
保存操作成功后，梦境便签通过缩小并朝底部 `Dreams` 导航收起的动画表达“已归档”，随后进入“梦已经保存”确认页，不自动开始理解。确认页直接展示刚保存的同一张梦境便签，并询问“现在开始理解吗？”。主操作为“现在理解”，次要操作为无边框文字链接“只保存，不解梦”；用户选择前不得创建 Understanding Session。不额外显示 `Dreams` 红点，避免在已明确的保存反馈后增加干扰。系统遵循操作系统的“减少动态效果”设置，以短淡出替代明显的位移动画。
...
AI 先生成一条用户能直接读懂的核心理解，再显示一条不确定信息；不输出长篇“解梦文章”。MVP 直接用简短引用呈现梦境片段和用户回答，不增加单独的“查看依据”交互；证据超过两条时才折叠更多内容。所有推断使用概率语言，并显示信息来源。
...
首屏最多显示三个行动项。点击“补充”后不新开流程，而是在当前草稿下展开 AI 对话输入区；原理解保持可见。AI 在对话文案中直接说明用户可以补充梦境细节、修正理解或说明现实联系，不再提供三组选项。用户可自由打字或语音补充，AI 最多追问 1-2 次；对话中提供内联文字“重新整理这次理解”，不显示“完成补充”大按钮。回到草稿后仍由“很贴近”确认是否保存。
