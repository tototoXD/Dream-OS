...
保存操作成功后，梦境便签通过缩小并朝底部 `Dreams` 导航收起的动画表达“已归档”，随后进入“梦已经保存”确认页，不自动开始理解。确认页明确询问“要现在开始理解这条梦吗？”，提供“现在开始理解”“稍后继续”“只保存，不理解”三个选择；用户选择前不得创建 Understanding Session。不额外显示 `Dreams` 红点，避免在已明确的保存反馈后增加干扰。系统遵循操作系统的“减少动态效果”设置，以短淡出替代明显的位移动画。
